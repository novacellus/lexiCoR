...
#' Base plot for word forms
#' 
#' Produces a base plot for further endorment
plot_form <- function(forms_cpos) {
  forms_plot <- ggplot(forms_cpos, aes(x = cposbin, group = form, colour = form))
}

plot_forms1 <- function(forms_cpos) {
  forms_plot <- plot_form(forms_cpos)
  formsplot1 <- forms_plot +
    geom_area(stat = "count",
              mapping = aes(fill = form ),
              alpha = 0.2) +
    theme_light()
  formsplot1
}
...
plot_forms2 <- function(forms_cpos) {
  forms_plot <- plot_form(forms_cpos) 
  forms_plot2 <-
    forms_plot + geom_line(stat = "count") +  theme_light()
  forms_plot2
}

plot_forms3 <- function(forms_cpos, forms_cpos_sum) {
  forms_plot <- plot_form(forms_cpos)
  forms_plot2 <-plot_forms2(forms_cpos)
  forms_plot3 <- forms_plot2 +
    ...
    geom_line(
      ...
      colour = "black",
      alpha = .5,
      group = 1, # Dummy variable overriding grouping by form
      stat = "identity"
    )
  forms_plot3
}
...
plot_forms_scatter <- function(forms_cpos) {
  ...
  cpos_forms_melted <-
    gather(cpos_forms, key = form, value = count, -bin)
  
  base_plot <-
    ggplot(
      data = filter(cpos_forms_melted, form != "total"),
      mapping = aes(
        x = rep(
          filter(cpos_forms_melted, form == "total")$count,
          number_of_forms
        ),
        y = count ,
        colour = form
...
meta_plot <- function (corpus = "PATROLOGIA", forms_cpos) {
  ...
  meta_all <- forms_cpos %>%
    ...
    mutate(., N = sapply(struc, function(x)
      cqi_struc2cpos("PATROLOGIA.text_id", x)[2] - cqi_struc2cpos("PATROLOGIA.text_id", x)[1] +
        1)) %>%
    mutate(., ratio = (count / N) * 1000000) # Per million words
  #Per author
  meta_author <-
    meta_all %>% group_by(., author) %>% summarise(., ratio = (sum(count) /
                                                                 sum(N)) * 1000000)
  meta_type <-
    meta_all %>% group_by(., type) %>% summarise(., ratio = (sum(count) / sum(N)) *
                                                   1000000)
  meta_periode <-
    meta_all %>% group_by(., periode) %>% summarise(., ratio = (sum(count) /
                                                                  sum(N)) * 1000000)
  ...
  meta_plot_auth_text <- ggplot(meta_all, mapping = aes(x = ratio)) +
    ...
    geom_line(
      stat = "density",
      data = meta_author,
      mapping = aes(x = ratio, colour = "authors")
    ) +
    scale_colour_manual(
      "Density in",
      breaks = c("texts", "authors"),
      values = c("red", "blue")
    )
  
  return(list(meta_plot = meta_plot_auth_text))
}